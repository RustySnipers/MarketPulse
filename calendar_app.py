...
class CalendarApp(tk.Frame):
    ...
    def create_widgets(self):
        ...
        for t in self.TYPE_COLORS:
            ...
            cb = tk.Checkbutton(filter_frame, text=t.capitalize(), variable=var, command=self.apply_filters)
            cb.pack(anchor='w')
            self.type_vars[t] = var
        tk.Label(filter_frame, text="Company:").pack(anchor='w')
        self.company_var = tk.StringVar()
        self.company_box = ttk.Combobox(filter_frame, textvariable=self.company_var, state='readonly')
        ...
        for sev, sym in self.SEVERITY_SYMBOLS.items():
            tk.Label(key_frame, text=f"{sym} = {sev.capitalize()} impact").pack(anchor='w')
    ...
    def apply_filters(self):
        ...
        self.filtered_events = [ev for ev in self.events if self.type_vars[ev['type']].get() and (selected_company == 'All' or ev['company'] == selected_company)]
    ...
    def refresh_calendar(self):
        ...
        for ev in self.filtered_events:
            ...
            event_id = self.cal.calevent_create(date, title, tags=ev['type'])
            self.cal.tag_config(ev['type'], background=self.TYPE_COLORS.get(ev['type'], 'white'))
            self.cal.tag_bind(ev['type'], '<Button-1>', self.show_event_details)
            item = f"{date} - {title}"
            self.day_event_box.insert(tk.END, item)
            self.day_event_box.itemconfig(tk.END, {'bg': self.TYPE_COLORS.get(ev['type'], 'white')})
            ToolTip(self.day_event_box, ev.get('description', ''))
